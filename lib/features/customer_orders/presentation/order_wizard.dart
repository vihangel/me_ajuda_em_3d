...
class _OrderWizardState extends State<OrderWizard> {
    ...
    try {
      final order = await widget.repository.createCustomerOrder(
        CreateCustomerOrderInput(
          ...
          quantity: _qty,
          hasReferenceImage: _pickedImage != null,
        ),
      );
      if (!mounted) return;
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('Pedido ${order.code} enviado!')));
  ...
  String _buildDesc() => [
    if (_catalogItem != null)
      'Catalogo: ${_catalogItem!.title} (${_catalogItem!.style})',
  ...
  Widget _buildStep() => switch (_step) {
    ...
    1 => StepDetails(
      ...
      onImageChanged: (v) => setState(() {
        if (!v) _pickedImage = null;
      }),
      ...
      children: [
        ...
        SizedBox(
          ...
          child: GradientButton(
            ...
            label: 'Comecar pedido',
            onPressed: () {
              if (categories.isNotEmpty) onSelect(categories.first);
            },
          ),
        ),
        const SizedBox(height: 24),

        // ── Título das categorias (redesigned) ──
        Text(
          'Escolha o tipo da peca',
        ...
        const SizedBox(height: 4),
        Text(
          'Voce pode ajustar os detalhes depois',
          style: TextStyle(color: colors.onSurfaceVariant),
        ),
        const SizedBox(height: 16),

        // ── Grid de categorias (redesigned — mais compacto, com descrição) ──
        GridView.builder(
          shrinkWrap: true,
          ...
          itemBuilder: (context, i) {
            ...
            return CategoryTile(
              ...
              title: categoryDisplayName(cat.icon, cat.title),
              subtitle: categoryShortDesc(cat.icon),
              accent: categoryAccent(cat.icon),
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  'Escolha a categoria, descreva\ne receba o orcamento.',
                  style: TextStyle(
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          _badge(Icons.flash_on_outlined, 'Atendimento\nrapido', colors),
        ],
      ),
    );
  }

  Widget _badge(IconData icon, String label, ColorScheme colors) {
    return Expanded(
      child: Column(
        mainAxisSize: MainAxisSize.min,
        children: [
...
// ---------------------------------------------------------------------------
// Category tile (redesigned — mais compacto, com descrição curta)
// ---------------------------------------------------------------------------
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(
      ...
      children: [
        ...
        const SizedBox(height: 4),
        Text(
          'Escolha um modelo base ou descreva do zero.',
          style: TextStyle(color: colors.onSurfaceVariant),
        ),
          ...
          child: Column(
            children: [
              ...
              DropdownButtonFormField<String>(
                ...
                onChanged: (v) =>
                    widget.onFinishChanged(v ?? 'Sem preferencia'),
              ),
            ],
          ),
        ),
        ...
        // Image
        ImageAttachCard(
          required_: widget.category.needsImage,
          attached: widget.hasImage,
          fileName: widget.pickedImageName,
          onTap: () async {
            if (widget.hasImage) {
              // Remove a imagem
              widget.onImageChanged(false);
              return;
            }
          ...
          return Container(
            ...
            child: Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    'Sem exemplos nessa categoria ainda.',
                    style: TextStyle(color: colors.onSurfaceVariant),
                  ),
                ),
              ],
